...
VAD_MODE = 2                # 0-3; higher values ignore background noise better but may miss quiet speech;
...
def record_voice():
    ...
    def callback(indata, frames, time, status):
        ...
        if is_speech:
            # If speech starts, add buffer and continue recording
            voiced_frames.extend(ring_buffer)   # add info in buffer to voiced_frames
            ring_buffer.clear()                 # empty buffer
            silence_count = 0                   # reset counter
        elif voiced_frames: 
            # increment silence_count if frame is silent
            silence_count += 1

        # Stop recording when max silent frames is reached
